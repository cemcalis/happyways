...
async function ensureReservationColumns() {
  ...
  const addColumn = async (name, type) => {
    if (!names.includes(name)) {
      await db.exec(`ALTER TABLE reservations ADD COLUMN ${name} ${type}`);
    }
  };

  await addColumn("user_name", "TEXT");
  await addColumn("user_email", "TEXT");
  await addColumn("user_phone", "TEXT");
  await addColumn("car_model", "TEXT");
  await addColumn("car_year", "INTEGER");
  await addColumn("car_image", "TEXT");
  await addColumn("pickup_location_id", "INTEGER");
  await addColumn("dropoff_location_id", "INTEGER");
  await addColumn("pickup_datetime", "TEXT");
  await addColumn("dropoff_datetime", "TEXT");
}
